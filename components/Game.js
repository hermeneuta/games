...
const Game = ({ games, handleNav, results }) => {
  ...
  return (
    <>
      {games !== "" ? (
        <div
          className={`font-serif shadow-md text-zinc-950 bg-gradient-to-bl from-lime-600 bg-lime-600 text-sm text-left flex-row items-center justify-center gap-10 m-auto mt-1 mb-1 p-4 rounded-md max-w-sm sm:max-w-md border border-lime-800 ${
            showText ? "" : "h-128"
          }`}
        >
